...
const getAllUserData = async (req, res) => {
  ...
  const all_user_data = await UserData.find({ user_id }).sort({
    createdAt: -1,
  });
...
const createUserData = async (req, res) => {
  try {
    ...
    const response = await openai.createCompletion({
      model: "text-davinci-002",
      prompt:
        "Give me feedback and tips on how to improve if I am being negative, if I am positive then give me words of encouragement to keep doing good and to check in daily with this application: " +
        transcript,
      temperature: 1,
      max_tokens: 200,
      top_p: 1,
    ...
    const response2 = await openai.createCompletion({
      model: "text-davinci-002",
      prompt:
        "Only give me a number, no other words, from 1-100 in terms of how positive the text is: " +
        transcript,
      temperature: 1,
      max_tokens: 100,
      top_p: 1,
    ...
    res
      ...
      .json({
        ...
        feedback: response.data.choices[0].text,
        score: response2.data.choices[0].text,
      });
  } catch (error) {
    console.log(error);
    res.status(400).json({ error: error.message });
...
const updateUserData = async (req, res) => {
  ...
  const user_data = await UserData.findOneAndUpdate(
    { _id: id },
    {
      ...req.body,
    }
